...
Initially we check if `metR` package includes all neccesary datasets and functions:
...
kable_styling(kable(row, "html"), position = "left",
font_size = 9) %>% row_spec(1, color = "black") 
}
```
...
We see very interesing regions below. 
...
In disease sample methylation rate starts from level 0.9 and deacreases to 0.15.
Curious observations is occuurance that at the beginning of region we see bigger methylation in control sample and at the end of region reverse case.
...
`preprocessing` function need two probes with specyfic columns:
...
This is a list of data.frames. One data.frame is result of running specyfic methods.
...
Other arguments are resposible for size of element on plots.
